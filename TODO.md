...
- [x] Add missing `@Entity` annotation to `Portfolio` and `Security` (src/main/java/com/wellsfargo/counselor/entity/Portfolio.java, Security.java) — both declare `@Id`/`@GeneratedValue` but Hibernate won't manage them as entities without `@Entity`.
- [x] Fix `Client.advisor` mapping in Client.java: it's annotated `@OneToMany(mappedBy = "client.advisor")` on a single `Advisor` field, which is invalid JPA; should be `@ManyToOne` since each client belongs to one advisor.
